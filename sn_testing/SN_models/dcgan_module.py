...
class Discriminator(nn.Module):
    def __init__(self, M=32):
        ...
        # M
        self.block1 = sn(nn.Sequential(
            nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=1),
            nn.LeakyReLU(0.1, inplace=False)
        ))
        self.block2 = sn(nn.Sequential(
            nn.Conv2d(64, 128, kernel_size=4, stride=2, padding=1),
            nn.LeakyReLU(0.1, inplace=False)
        ))
        # M / 2
        self.block3 = sn(nn.Sequential(
            nn.Conv2d(128, 128, kernel_size=3, stride=1, padding=1),
            nn.LeakyReLU(0.1, inplace=False)
        ))
        self.block4 = sn(nn.Sequential(
            nn.Conv2d(128, 256, kernel_size=4, stride=2, padding=1),
            nn.LeakyReLU(0.1, inplace=False)
        ))
        # M / 4
        self.block5 = sn(nn.Sequential(
            nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1),
            nn.LeakyReLU(0.1, inplace=False)
        ))
        self.block6 = sn(nn.Sequential(
            nn.Conv2d(256, 512, kernel_size=4, stride=2, padding=1),
            nn.LeakyReLU(0.1, inplace=False)
        ))
        # M / 8
        self.block7 = sn(nn.Sequential(
            nn.Conv2d(512, 512, kernel_size=3, stride=1, padding=1),
            nn.LeakyReLU(0.1, inplace=False)
        ))

        self.linear = sn(nn.Linear(M // 8 * M // 8 * 512, 1))
        self.initialize()
